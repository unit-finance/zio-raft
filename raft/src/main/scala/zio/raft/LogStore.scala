...
object LogStore:
  ...
  def getLogs(from: Index, toInclusive: Index) =
    ZIO.service[LogStore].flatMap(_.getLogs(from, toInclusive))
  def storeLog(logEntry: LogEntry) =
    ZIO.service[LogStore].flatMap(_.storeLog(logEntry))
  ...
  def deleteFrom(minInclusive: Index) =
    ZIO.service[LogStore].flatMap(_.deleteFrom(minInclusive))
end LogStore
