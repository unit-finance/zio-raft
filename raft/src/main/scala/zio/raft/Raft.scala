...
class Raft(memberId: MemberId, peers: Peers, state: Ref[State]):
  ...
  def makeElectionTimeout =
    for
      ...
      interval <- zio.random.nextIntBetween(electionTimeout, electionTimeout * 2).map(_.millis)
  ...
  def stepDown(term: Term) =
    for
      ...
      _ <- state.update(s => State.Follower(s.commintIndex, s.lastApplied, electionTimeout))
    yield term
  ...
  ): URIO[Has[Stable] & Has[LogStore] & Clock & Random, RequestVoteResult] =
    for
      ...
      result <-
        if (
          currentTerm == r.term && (votedFor.contains(r.candidateId) || votedFor.isEmpty) &&
          (r.lastLogTerm > lastTerm ||
            (r.lastLogTerm == lastTerm &&
              r.lastLogIndex >= lastIndex))
        ) then resetElectionTimer.as(RequestVoteResult.Granted(memberId, currentTerm))
      ...
      _ <- s match
        case candidate: State.Candidate =>
          r match
            ...
            case g: RequestVoteResult.Granted  => state.set(candidate.addVote(g.from))
        case _ => ZIO.unit
    yield ()

  def handleAppendEntriesRequest(
      r: AppendEntriesRequest
  ): URIO[Has[Stable] & Has[LogStore] & Clock & Random, AppendEntriesResult] =
    for
      currentTerm <- Stable.currentTerm
      currentTerm <-
        if currentTerm < r.term then stepDown(r.term)
      ...
      result <-
        if currentTerm != r.term then ZIO.succeed(AppendEntriesResult.Failure(memberId, currentTerm))
        else
          for
            _ <- convertToFollower
            lastIndex <- LogStore.lastIndex
            ...
            result <-
              if r.previousIndex.isZero || (r.previousIndex <= lastIndex && localPreviousLogTerm == r.previousTerm) then
                for
                  // TODO: omptimize this code
                  matchIndex <- ZIO.foldLeft(r.entries)(r.previousIndex.plusOne)((index, entry) =>
                    for
                      logTerm <- LogStore.logTerm(index)
                      _ <- ZIO.when(logTerm != entry.term)(LogStore.deleteFrom(index))
                      _ <- LogStore.storeLog(entry)
                    yield index.plusOne
                  )

                  _ <- updateCommitIndex(commitIndex => Index.max(commitIndex, r.leaderCommitIndex))
                yield AppendEntriesResult.Success(memberId, currentTerm, matchIndex)
              else ZIO.succeed(AppendEntriesResult.Failure(memberId, currentTerm))
          yield result
    yield result
  ...
  def handleAppendEntriesReply(r: AppendEntriesResult) =
        ...
        case leader: State.Leader if currentTerm == r.term =>
          r match
            ...
            case AppendEntriesResult.Failure(from, _) =>
              ...
              state.set(
                leader.withNextIndex(from, Index.max(Index.one, nextIndex.minusOne)).withRPCDueNow(from)
              )
        case _ => ZIO.unit
    yield ()
    ...
    def startNewElectionRule =
      def start =
        for
          ...
          electionTimeout <- makeElectionTimeout
          _ <- state.update(s =>
            State.Candidate(RPCDue.makeNow(peers), 1, s.commintIndex, s.lastApplied, electionTimeout)
          )
        yield ()
    ...
    def becomeLeaderRule =
      ...
      for
        s <- state.get
        _ <- s match
          case c: State.Candidate if c.voteGranted > numberOfServers / 2 => becomeLeader(c)
          case _                                                         => ZIO.unit
      yield ()
    ...
    def advanceCommitIndexRule =
      for
        ...
        _ <- s match
          case l: Leader =>
            val matchIndexes = (lastIndex :: l.matchIndex.indices).sortBy(_.value)
            val n = matchIndexes(numberOfServers / 2)
            for
              nTerm <- LogStore.logTerm(n)
              _ <- ZIO.when(nTerm == currentTerm)(state.set(l.withCommitIndex(Index.max(n, l.commintIndex))))
            yield ()
          case _ => ZIO.unit
      yield ()
        ...
        _ <- s match
          case l: Leader if l.heartbeatDue.due(now, peer) =>
            ...
            for
              ...
              _ <- state.set(
                l.withHeartbeaetDue(peer, now.plus((electionTimeout / 2).millis))
              )
            yield ()
          case _ => ZIO.unit
      yield ()
    ...
    def sendAppendEntriesRule(peer: MemberId) =
      for
        ...
        _ <- s match
          case l: Leader if l.rpcDue.due(now, peer) && leaderLastIndex >= l.nextIndex.get(peer) =>
            val nextIndex = l.nextIndex.get(peer)
            val previousIndex = nextIndex.minusOne
            val lastIndex = Index.min(previousIndex.plus(batchSize), leaderLastIndex)
            ...
            for
              ...
              _ <- state.set(
                l.withRPCDue(peer, now.plus(rpcTimeout)).withHeartbeaetDue(peer, now.plus((electionTimeout / 2).millis))
              )
            yield ()
          case _ => ZIO.unit
      yield ()
        ...
        _ <- s match
          case c: Candidate if c.rpcDue.due(now, peer) =>
            for
              ...
              _ <- RPC.sendRequestVote(peer, RequestVoteRequest(currentTerm, memberId, lastIndex, lastIndexTerm))
            yield ()
          case _ => ZIO.unit
      yield ()
    ...
    def preRules =
      for
        ...
        _ <- ZIO.foreach_(peers)(p => sendRequestVoteRule(p) &&& sendHeartbeatRule(p))
      yield ()
    ...
    def postRules =
      for
        ...
        _ <- ZIO.foreach_(peers)(p => sendAppendEntriesRule(p) &&& sendRequestVoteRule(p))
      yield ()

    def handleMessage(message: RPCMessage): UIO[Unit] = ???
    ...
    def sendCommand(command: Command): UIO[Response] = 
      for {
        promise <- zio.Promise.make[Nothing, Response]
        _ <- commandsQueue.enqueue((command, promise))
        result <- promise.await
      } yield result
            
    def run = 
      val tick = ZStream.repeat(StreamItem.Tick)
      val messages = ZStream.fromQueue(RPC.incomingMessages).map(StreamItem.Message.apply)

      ZStream.mergeAllUnbounded(16)(tick, messages).foreach(handleStreamItem)
