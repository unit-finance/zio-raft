...
object RaftIntegrationSpec extends MutableRunnableSpec(zio.clock.Clock.live):
        ...
        for       
            res <- makeRaft().use{
                ...
                for 
                    ...
                    _ <- ZIO.debug(s"before leader is ${beforeLeader.memberId}")
                    //stop the leader
    ...
    testM("take snapshot"):
        for       
            res <- makeRaft(true).use{
            case (r1,rpc1, killSwitch1, r2, rpc2, killSwitch2, r3, rpc3, killSwitch3) => {
                for 
                    _ <- ZIO.sleep(5.seconds).provideLayer(zio.clock.Clock.live)
                    leader <- findTheLeader(r1, r2, r3).debug("----------1")                    
                    _ <- leader.sendCommand(Increase).debug("----------2")
                    _ <- leader.sendCommand(Increase).debug("----------3")
                    _ <- leader.sendCommand(Increase).debug("----------4")
                    x <- leader.sendCommand(Get).debug("----------5")
                yield (x)
            }
            }.provideLayer(zio.ZEnv.live ++ logging)
        yield assertTrue(res == 3)