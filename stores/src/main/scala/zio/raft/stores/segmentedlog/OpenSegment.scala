...
class OpenSegment[A <: Command: Codec](
  ...
  def getLastTermIndex[A <: Command: Codec] =
    for {
      last <- makeStream(channel).via(recordsOnly).run(lastAndDecode).orDie
    } yield last match
      case None    => (previousTerm, firstIndex.minusOne)
      case Some(e) => (e.term, e.index)

  def writeEntry(entry: LogEntry[A]) = writeEntries(List(entry))
  ...
  def writeEntries(entries: List[LogEntry[A]]) =
    for {
  ...
  def deleteFrom(minInclusive: Index) =
    for
  ...
  def recoverFromCrash =
    for
      lastChecksum <- makeStream(channel, validateChecksum = true)
        .collect:
      ...
      _ <- lastChecksum match
        ...
        case Some(BaseTransducer.Result.Checksum(_, false)) =>
          for
            ...
            _ <- lastGoodChecksumOffset match
              case None =>
                val offset = BaseTransducer.headerSize
                for
                  _ <- ZIO.logInfo("SegementedLog: No valid checksum found - truncate to header")
                  _ <- channel.truncate(offset).orDie
                  _ <- channel.force(true).orDie
                  _ <- positionRef.set(offset)
                yield ()
              case Some(checksumOffset) =>
                val offset = checksumOffset + BaseTransducer.isEntrySize + BaseTransducer.checksumSize
                for
                  _ <- ZIO.logInfo(s"SegementedLog: Truncate to last valid checksum at offset ${checksumOffset}")
                  _ <- channel.truncate(offset).orDie
                  _ <- channel.force(true).orDie
                  _ <- positionRef.set(offset)
                yield ()
