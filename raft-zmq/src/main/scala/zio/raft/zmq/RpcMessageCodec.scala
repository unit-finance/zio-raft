...
object RpcMessageCodec:        
    ...
    private def chunkCodec = variableSizeBytes(int32, bytes).xmap(bv => Chunk.fromIterable(bv.toIterable), c => ByteVector(c.toIterable))
    ...
    private def installSnapshotCodec[A <: Command] = (termCodec :: memberIdCodec :: indexCodec :: termCodec :: int64 :: bool(8) :: chunkCodec).as[InstallSnapshotRequest[A]]

    private def installSnapshotResultCodec[A <: Command] = discriminated[InstallSnapshotResult[A]].by(uint8)
