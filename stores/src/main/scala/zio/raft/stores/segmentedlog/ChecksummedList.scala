...
class ChecksummedList[A](codec: Codec[A]) extends Codec[List[A]]:
    ...
    if failure == null then
      ...
      def merge(offset: Int, size: Int): BitVector = size match
        ...
        case _ =>
          ...
          ) // TODO (eran): look into this merge function a bit more, need to better undertand this
  ...
  private def entryDecoder =
    entrySizeCodec
      .flatZip(size => peek(bits(size / 8)))
      .flatMap((size, bits) => fixedSizeBytes(size, codec).map(a => (bits, a)))
