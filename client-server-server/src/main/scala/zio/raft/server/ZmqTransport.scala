...
trait ZmqTransport {
  ...
  /**
   * Stream of incoming messages from clients with routing IDs.
   */
   ...
   * 
   * @param routingId Target client's routing ID
...
object ZmqTransport {
  ...
  def make(config: ServerConfig): ZIO[ZContext & Scope, Throwable, ZmqTransport] = 
    for {
      ...
      transport = new ZmqTransportImpl(
        socket,
        messagesSent,
        messagesReceived,
        sendErrors,
  ...
  /**
   * Internal implementation of ZmqTransport.
   */
  private class ZmqTransportImpl(
    socket: ZSocket,
  ...
  ) extends ZmqTransport {
    
    override def incomingMessages: ZStream[Any, Throwable, (RoutingId, ClientMessage)] = {
      ZStream
    ...
    override def sendMessage(routingId: RoutingId, message: ServerMessage): IO[Throwable, Unit] = {
      for {
        // Serialize message
    ...
    override def getStats(): UIO[ZmqTransportStats] = 
      for {
