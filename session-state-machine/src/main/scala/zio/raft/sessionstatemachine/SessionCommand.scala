...
/** Wrapper for server-initiated requests that users emit via StateWriter.log().
  *
  * Users must wrap their server request payload with the target sessionId. This allows server requests to target ANY
  * session, not just the current one.
  *
  * @param sessionId
  *   The session this request is for (may be different from current session!)
  * @param payload
  *   The actual server request payload
  *
  * @example
  *   {{{
...
/** Server request with assigned ID after being added to state.
  ...
  * @param requestId
  *   The unique request ID (assigned by SessionStateMachine)
  * @param payload
  *   The actual server request payload
  */
