  ...
  override def spec: Spec[TestEnvironment, Any] = suite("PendingReads Spec")(
    ...
    test("withHeartbeatResponse - completes heartbeats with majority and preserves others") {
      for
        ...
        result1 <-
          promise1.await.either.timeout(zio.Duration.fromMillis(100)).map(_.getOrElse(Left(NotALeaderError(None))))
        result2 <-
          promise2.await.either.timeout(zio.Duration.fromMillis(100)).map(_.getOrElse(Left(NotALeaderError(None))))
        isDone3 <- promise3.isDone
    ...
    test("withHeartbeatResponse - maintains sorted order after partial completion") {
      for
        ...
        result2 <-
          promise2.await.either.timeout(zio.Duration.fromMillis(100)).map(_.getOrElse(Left(NotALeaderError(None))))
        result3 <-
          promise3.await.either.timeout(zio.Duration.fromMillis(100)).map(_.getOrElse(Left(NotALeaderError(None))))
        isDone4 <- promise4.isDone
